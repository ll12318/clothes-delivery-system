...
// GetRouteDataSource 获取Route的数据源
// @Tags Route
// @Summary 获取Route的数据源
// @accept application/json
// @Produce application/json
// @Success 200 {object} response.Response{data=object,msg=string} "查询成功"
// @Router /route/getRouteDataSource [get]
func (routeApi *RouteApi) GetRouteDataSource(c *gin.Context) {
    // 此接口为获取数据源定义的数据
   if dataSource, err := routeService.GetRouteDataSource(); err != nil {
        global.GVA_LOG.Error("查询失败!", zap.Error(err))
   		response.FailWithMessage("查询失败", c)
   	} else {
